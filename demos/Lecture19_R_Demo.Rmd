...
The following code first grabs the rownames of the `state.x77` table, them converts `state.x77` to a `tibble` called `state_date`, and then adds that column with the state names and makes them lower case:
...
# our new state_plot_data dataset:
...
Unfortunately, some popular projections (like the Robinson projection and the Winkel Tripel projection) are not readily available in the `sp` package (which is what we have been using throughout this example). It's a little bit annoying to get these projections working in `R`, but it can be done: See [this tutorial](https://wilkelab.org/practicalgg/articles/Winkel_tripel.html). There isn't one "right" projection, but some are better than others. We won't go into details about implementing different projetions in `R`, but teams working on spatial data are encouraged to read up on projections if they feel strongly about this issue.
...
#                       state_plot_data_rand$Illiteracy <- sample(state_plot_data_rand$Illiteracy)
#                       # Return the plot 
#                       get_state_map_illit(state_plot_data_rand)
#                     })
...
# Versus the old school way of doing this...
# library(gridExtra)
...
# BONUS: Uniform size with [`statebins`](https://github.com/hrbrmstr/statebins)
...
+ The `sp` package was the main package for awhile, but now it's been replaced by the [`sf` package](https://r-spatial.github.io/sf/index.html)

+ [Tutorial for displaying county level maps](https://socviz.co/maps.html#maps), from an [excellent data viz book](https://socviz.co/index.html#preface) that's free online. Just note that it's syntax for the `statebins` package is out of date.

+ [More info on map-making](https://clauswilke.com/dataviz/geospatial-data.html) from another great textbook from the creator of the [`cowplot` package](https://cran.r-project.org/web/packages/cowplot/vignettes/introduction.html).
