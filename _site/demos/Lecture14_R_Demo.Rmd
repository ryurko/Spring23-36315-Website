...
For the first part of this demo we'll use a dataset of shots by [LeBron James](https://en.wikipedia.org/wiki/LeBron_James) during this year's NBA season. _For people that are interested, this was constructed using the [`hoopR`](https://hoopr.sportsdataverse.org/)_.
...
It's really easy to add a two-dimensional density (via contour lines) to the plot: We just add `geom_density2d()`:
...
We might also want to change the bandwidth. In 2D kernel density estimation, we must specify two bandwidths -- one for the x-direction, one for the y-direction. We'll see how to do this on the homework.
