...
Similar to the lecture 22 demo, we need to pre-process the `stranger_things_text` dataset into a tokenized form. Again, we'll use the [`tidytext` package](http://tidytextmining.com/) to convert the `dialogue` column into individual words as the tokens.
...
Before we continue to explore this dataset, we will again remove stop words and perform stemming like we did in the Lecture 22 R Demo. First, we load a table of stop words and simply filter them out from our data:
...
Using our usual `tidyverse` syntax, we will compute the frequency of each `stem` in each episode:
...
The most common `R` package for implementing topic models is called, unsurprisingly, `topicmodels`. However, the `topicmodels` package requires a `DocumentTermMatrix` object. To address this, we can use `tidytext`'s `cast_dtm()` function on our `st_episode_stem_summary` table from before - treating each episode as a separate document:
...
Notice that this summary output displays information about the matrix, reflecting the high sparsity (proportion of 0s) in the matrix at 89% - due to the high number of unique words/tokens (5602). We are being a bit lazy here, since numbers are included - but we could remove those if we wanted to.
...
+ For each topic, the probability of a certain word occurring (there are 5602 words/stems, so there will be 5602 of these for each topic). These probabilities will be called `beta`.
...
This visualization attempts to help us understand the two topics that were extracted from the episodes' words/stems. Obviously, there is noticeable overlap between these topics as both feature "yeah", "hei" (stem for "hey"), "uh", and other frequently used words across the show. This is an advantage of topic modeling as opposed to "hard clustering" methods: topics used in natural language could have some overlap in terms of words. ("Hard clustering" methods would force each word to be in a single topic, rather than letting there be overlap between topics.) But there are differences, such as "mom" and "mike" in Topic 1 versus "max" and "eddi" in Topic 2. This might hint at Topic 2 being associated with later seasons of the show - such as the rather long and verbose Season 4...
...
```{r topiccompare, dependson = "beta_spread"}
beta_spread %>%
...
Each of these values is an estimated proportion of words from that document that are generated from that topic. For example, the model estimates that basically `r scales::percent(st_documents$gamma[1])` of the words in episode 1 were generated from Topic 1. In comparison, for the first episode in season 4 (`4_1`), about 73% of the words were generated from Topic 1.  
