...
To conduct PCA, __you must center and standardize your variables__. We can either do that manually with the `scale()` function:
...
As you can see from just the first so many rows, these matrices match. If we view the dimensionality of this matrix (just the one returned to us by `prcomp`), we can see that it matches the dimensionality of the original dataset:
...
scree plot manually. All we need to do is grab the proportion of variance explained by each component, turn it into a table, and then display it in some way. We already computed these values earlier in the demo, but we also just grab the singular values directly provided to us by `R`:

```{r}
# Manual creation of elbow plot, start by computing the eigenvalues and dividing by
# the total variance in the data (see lecture notes):
